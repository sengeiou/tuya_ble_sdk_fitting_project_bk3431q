...
void show_ke_mem(void)
{
	...
	{
		UART_PRINTF("ke_env.heap_used[%d] = 0x%x,free_size = 0x%x\r\n",cursor,ke_env.heap_used[cursor],ke_env.heap[cursor]->free_size); 
	}
	UART_PRINTF("ke_env.max_heap_used = 0x%x\r\n",ke_env.max_heap_used);
	#endif // 
}
...
//		UART_PRINTF("RWIP_HEAP_NON_RET_SIZE = 0x%x\r\n",RWIP_HEAP_NON_RET_SIZE);
    ...
    #if SYSTEM_SLEEP///(NVDS_SUPPORT && SYSTEM_SLEEP)///0722
	...
	rwip_env.sleep_enable = true;
	rwip_env.ext_wakeup_enable = true;
    rwip_env.wakeup_delay=4;//16;//0722
	#endif		
...
void rwip_reset(void)
{
    ...
    #if 0//(NVDS_SUPPORT && SYSTEM_SLEEP)
    // Activate deep sleep feature if enabled in NVDS
    if(nvds_get(NVDS_TAG_SLEEP_ENABLE, &length, &sleep_enable) == NVDS_OK)
    {
        if(sleep_enable != 0)
