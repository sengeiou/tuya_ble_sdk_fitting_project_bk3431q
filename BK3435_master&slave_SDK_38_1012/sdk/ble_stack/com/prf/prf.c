    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        for(i = 0; i < BLE_NB_PROFILES ; i++)
        {
            if(prf_env.prf[i].id == params->prf_task_id)
            {
                ...
                UART_PRINTF("GAP_ERR_NOT_SUPPORTED i:%d,prf_task_id:0x%x",i,params->prf_task_id);
                break;
            }
        }
    }
...
//                    UART_PRINTF("prf_add_profile i = %d*prf_task :0x%04x\r\n",i,*prf_task);
//                    UART_PRINTF("default_handler = 0x%x\r\n",prf_env.prf[i].desc.default_handler);
                }
                break;
            }
        }
...
ke_task_id_t prf_get_task_from_id(ke_msg_id_t id)
{
    ...
    {
        // check if profile identifier is known
        if(KE_TYPE_GET(prf_env.prf[i].id) == id)
        {
