...
void spi_init(uint8_t freq_div)
{
	 ...
	 // Enable GPIO P0.4, P0.5, P0.6, P0.7 peripheral function for spi
        ...
        //GPIO_SPI_CFG |= (((0x1<<GPIO_SPI_SCK_PIN)<<16) | ((0x1<<GPIO_SPI_MOSI_PIN)<<24));
        //GPIO_SPI_CFG |= (((0x1<<GPIO_SPI_SCK_PIN) | (0x1<<GPIO_SPI_MOSI_PIN))<<16);
      	UART_PRINTF("SPI_CTRL 0 = %x\r\n",REG_APB2_SPI_CTRL);
		
		REG_APB2_SPI_CTRL = (0x01UL << BIT_SPI_CTRL_SPIEN)
					|( 0x1 << BIT_SPI_CTRL_MSTEN)           //master mode 
					| (CKPHA_CLK2 << BIT_SPI_CTRL_CKPHA)
					| (CKPOL_H << BIT_SPI_CTRL_CKPOL)
...
void hw_spi_write(uint8_t *wbuf, uint32_t w_size)
{
    spi_tx_start();
    while(w_size > 0)
    {
...
void hw_spi_read(uint8_t *rbuf, uint16_t *r_size)
{
    uint8_t rsp_data = 0;
    uint16_t read_len = 0;
    ...
    while(rsp_data != 0x55)
    {
        Delay_us(12);
        spi_write_byte(0x00);
        rsp_data = spi_read_byte();//spi_read_data(rsp_data, 1);
...
void hw_spi_test1(uint8_t *data, uint8_t len)
{
    uint8_t rsp_data[255];
    uint16_t rsp_len = 0;
    UART_PRINTF("hw spi data:\r\n");
    hw_spi_write(data, len);
    hw_spi_read(rsp_data, &rsp_len);
...
//static uint8_t electric_esam_get_info[6] = {0x80, 0x0E, 0x00, 0x06, 0x00, 0x00};///, 0x00, 0x5f};
//static uint8_t electric_esam_get_info[8] = {0x55, 0x00, 0xa0, 0x01, 0x01, 0x00, 0x00, 0x5f};//55 + 90 00 + len + data
void hw_spi_test(uint8_t *data, uint8_t len)
{
    uint8_t rsp_data[255];
    //spi_init(2);
    ...
    spi_write_read(data, len, NULL, 0, 0);
    spi_write_read(NULL, 0, rsp_data, 50, 0);
