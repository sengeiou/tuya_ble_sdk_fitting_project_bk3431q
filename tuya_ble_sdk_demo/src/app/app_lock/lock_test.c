		...
		case LOCK_TEST_CMD_ENTER: {
            if(len == 0)
            {
                if(app_port_get_connect_status() != BONDING_CONN) {
                    ...
                    TUYA_APP_LOG_INFO("lock test mode enter fail!", cmd);
                    TUYA_APP_LOG_INFO("The device is bonded.", cmd);
...
LOCK_TEST_PARAM_ERROR:
    if(is_param_error)
    {
        TUYA_APP_LOG_INFO("LOCK_TEST_PARAM_ERROR! Please check your param.");
        TUYA_APP_LOG_INFO("Error cmd: %d", cmd);
        lock_test_mode_exit();
    }
}
...
static void lock_test_mode_enter(void)
{
    TUYA_APP_LOG_INFO("lock test mode enter success!");
...
static void lock_test_mode_exit(void)
{
    TUYA_APP_LOG_INFO("lock test mode exit success!");
...
static void lock_test_rsp(uint16_t sub_cmd, uint8_t* buf, uint16_t size)
{
    ...
    tuya_ble_production_test_asynchronous_response(1, rsp, rsp_len);
}
